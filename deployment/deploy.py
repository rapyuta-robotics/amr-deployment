...
def deploy_intelligence(client, config):
    ...
    if not rio_gwm_deployment:
        ...
        rio_gwm_deployment.poll_deployment_till_ready(
            retry_count=150, sleep_interval=6)
...
def deploy_simulation(client, config):
    ...
    for i in range(len(config['AMR_PARAMS'])):
        amr_deployments[i].poll_deployment_till_ready(
            retry_count=1000, sleep_interval=10)
        print(" AMR " + str(i) + " deployed successfully")
...
def deprovision(client, config):
    ...
    db_deployment_name = config['deployment_prefix'] + '_postgress'
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Deployment script for hitachi')
    parser.add_argument(
        '--prefix',
        default=None,
    ...
    if args.deprovision:
        ...
        if args.destroy:
            update_packages(client, config, True)
